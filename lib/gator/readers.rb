  ...
  module Gator
    module Readers
      ...
      def range(date, grain = DEFAULT_GRAIN, opts = {})
        ...
        result_set.sort_by { |di| di['date'] }
      end

      # Group_by - retuns a collection for a specific key
      def group_by(date, _grain, opts = {})
        # Get Offset
        off_set = if date.is_a?(Range)
                    date.last.utc_offset
                  else
                    date.utc_offset
        end
        aggr = collection_for_group(date, HOUR, 0, opts)

        # iterate and accumulate the data
        aggr.inject([]) do |data, item|
          (data << item)
        end
      end
    end
